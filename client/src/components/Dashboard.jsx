    ...
    axios
      ...
      .then((result) => {
        ...
        const filteredUsers = result.data.filter(
          (user) => user.userOrders[0]?.cart?.length > 0
        );
      ...
      {data.map((user, index) => (
        <div key={index} className="mb-12 bg-white p-6 rounded-2xl shadow-md">
          <div className="flex  mb-5 ">
            ...
            <div className="text-2xl absolute right-12 font-semibold">Total: Rs. {user.userOrders[0]?.totalPrice}</div>
          </div>

          {user.userOrders[0]?.cart?.length > 0 ? (
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {user.userOrders[0]?.cart?.map((item, idx) => (
                    <tr
                      key={idx}
